...
const Subsection = ({
  ...
  const sortTags = React.useCallback(
    (a: TagProps, b: TagProps) => {
      if (tagTextIsFiltered(a, filters) && tagTextIsFiltered(b, filters)) {
        ...
        tagIconIsFiltered(a, filters) &&
        tagIconIsFiltered(b, filters)
      ) {
        return a.text.localeCompare(b.text);
  ...
  return (
    <div className="subsection">
      ...
      {description && (
        <span className="subsection-description">
          {description}
          <br />
        </span>
      )}
      {(contentShouldBeFiltered ? tags?.sort(sortTags) : tags)?.map((tag) =>
        buildTagWithFilters(tag)
      )}
    </div>
  );
};

const subsectionShouldRender = (
  ...
  subsection.tags.find(
    (tag) => !tag.isFilterable || tagIsFiltered(tag, filters)
  );
...
export const Section = ({ title, subsections }: SectionProps) => {
  ...
  return (
    <div className="section">
      ...
      {placeBetween(
        subsections
          .filter((item) => subsectionShouldRender(item, filters))
          .map((item) => <Subsection {...item} />),
        <hr />
      )}
    </div>
  );
